...
import type { AppState, BinaryFiles, StaticCanvasAppState } from "../types";
...
/**
 * Export at exact pixel dimensions - dimensions are in pixels (not inches)
 * This will scale the canvas content to fit the exact dimensions provided
 */
export const exportExact = async (
  elements: readonly NonDeletedExcalidrawElement[],
  dimensions: { w: number; h: number },
  dpi: number = 300,
  files: BinaryFiles = {},
) => {
  // First, get the natural size of the elements
  await Fonts.loadElementsFonts(elements);
  
  const frameRendering = getFrameRenderingConfig(null, null);
  const elementsForRender = prepareElementsForRender({
    elements,
    exportingFrame: null,
    exportWithDarkMode: false,
  ...
  const [minX, minY, naturalWidth, naturalHeight] = getCanvasSize(
    getRootElements(elementsForRender),
    0,
  );

  // Determine export dimensions - use original image size if available
  let exportWidth = dimensions.w;
  let exportHeight = dimensions.h;
  const imageElements = getInitializedImageElements(elementsForRender);
  
  if (imageElements.length > 0) {
    const firstImage = imageElements[0];
    const imageData = files[firstImage.fileId];
    
    if (imageData) {
      try {
        const img = await loadHTMLImageElement(imageData.dataURL);
        // Use the original image size - maintains native resolution
        exportWidth = img.naturalWidth;
        exportHeight = img.naturalHeight;
      } catch (error) {
        console.warn("Could not load image dimensions, using provided dimensions:", error);
      }
    }
  }

  // Calculate the scale needed to fit the export dimensions
  const scaleX = exportWidth / naturalWidth;
  const scaleY = exportHeight / naturalHeight;
  // Use the smaller scale to maintain aspect ratio
  const scale = Math.min(scaleX, scaleY);
  
  // Create a canvas with the exact export dimensions
  const canvas = document.createElement("canvas");
  ...
  // Render directly to our sized canvas
  renderStaticScene({
    canvas,
    rc: rough.canvas(canvas),
    elementsMap: toBrandedType<RenderableElementsMap>(
    ...
    appState: {
      ...
      scrollX: -minX + offsetX,
      scrollY: -minY + offsetY,
      zoom: defaultAppState.zoom,
      shouldCacheIgnoreZoom: false,
      theme: "light" as any,
      ...
      currentHoveredFontFamily: defaultAppState.currentHoveredFontFamily,
      hoveredElementIds: {},

...
import { exportExact } from "../scene/export";
...
const ImageExportModal = ({
  ...
  const handlePrintExportJpeg = async () => {
    try {
      // Get the canvas dimensions from the preview
      const previewCanvas = previewRef.current?.querySelector('canvas') as HTMLCanvasElement | null;
      if (!previewCanvas) {
        console.error("No preview canvas found");
        return;
      }

      // Get the actual pixel dimensions from the preview canvas
      const w = previewCanvas.width;
      const h = previewCanvas.height;
      
      // Export at exact canvas dimensions, passing files to render images
      const canvas = await exportExact(exportedElements, { w, h }, printDpi, files);
      
      // Convert PNG to JPEG at 0.95 quality
      canvas.toBlob(
        (blob) => {
          if (!blob) return;

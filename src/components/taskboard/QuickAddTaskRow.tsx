...
export function QuickAddTaskRow({ status, projects, onSave, onCancel }: QuickAddTaskRowProps) {
  ...
  return (
    <tr ref={rowRef} className="border-t border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
      {/* Status dot placeholder */}
      <td className="px-3 py-2 text-center">
        <div className="w-4 h-4 rounded-full border-2 border-gray-400 dark:border-gray-500 bg-transparent" />
      </td>
      ...
      <td colSpan={5} className="px-3 py-2">
        ...
        <div 
          className="text-xs text-gray-600 dark:text-gray-400 font-semibold mb-0.5 cursor-pointer hover:text-gray-900 dark:hover:text-gray-100 relative"
        ...
        {showProjectPopover && (
          <>
            ...
            <div className="absolute left-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded shadow-lg z-50 p-2 min-w-[240px] max-h-64 overflow-y-auto">
              {projects.map((project) => {
                ...
                return (
                  <button
                    ...
                    className="w-full text-left px-2 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded text-xs transition-colors"
                  >
                    {display}
                  </button>
                );
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="flex-1 text-sm font-medium bg-transparent border-0 outline-none focus:ring-0 p-0 dark:text-gray-100 dark:placeholder:text-gray-500"
            autoFocus
          />
          ...
          <div className="flex items-center flex-shrink-0 mr-2 gap-1 relative">
            {assignees.length === 0 ? (
              ...
              <>
                ...
                {assignees.length > 2 && (
                  <div 
                    className="w-6 h-6 rounded-full bg-gray-400 dark:bg-gray-600 border-2 border-white dark:border-gray-800 flex items-center justify-center text-white font-semibold cursor-pointer ml-0.5 text-[9px]"
                    onClick={() => setShowAssigneePopover(true)}
                  >
                    +{assignees.length - 2}
                  </div>
          ...
          {showAssigneePopover && (
            <>
              ...
              <div className="absolute right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded shadow-lg z-50 p-2 min-w-fit max-h-64 overflow-y-auto">
                  ...
                  projectMembers.map((member) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-2 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded text-xs transition-colors"
                      >
                        <UserAvatar 
                          user={{
                            name: member.userName,
                          ...
                          size="xs" 
                        />
                        <span className="flex-1 text-left text-gray-700 dark:text-gray-300">{member.userName}</span>
